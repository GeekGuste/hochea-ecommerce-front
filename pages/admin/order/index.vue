        ...
        <table class="table table-stripped">
            ...
            <tbody>
                <tr v-for="order in orderList.results" :key="order.id">
                    ...
                    <td>
                        <nuxt-link :to="'/admin/order/' + order.id + '/details'" class="btn btn-primary">Détails</nuxt-link>
                        <button v-if="!order.is_delivered" @click="confirmDelivery(order.id)" class="btn btn-success">Confirmer la livraison</button>
                    </td>
                </tr>
            </tbody>
            </table>
...
export default Vue.extend({
  ...
  methods: {
    ...
    loadOrders(){
        this.$axios.$get("/api/order/all/").then((orderList: PaginatedList<Order>) => {
